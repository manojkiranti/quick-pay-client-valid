<template>
  <div class="card-form-wrap">
    <div class="row">
      <div class="col-md-12">
        <form novalidate @submit.prevent="submitForm">
          <div class="mb-4">
            <md-field>
              ...
              <md-select
                ...
                @md-selected="verifyAlreadyExist"
              >
          ...
          <template v-if="form.cardType == 'CREDIT'">
            ...
            <div class="mb-4">
              <md-field :class="getValidationClass('cardExipryDate')">
                ...
                <span
                  class="md-error"
                  v-else-if="!$v.form.cardExpiryDate.minLength"
                  >Invalid Date</span
                >
              </md-field>
            </div>
          ...
          <div class="form-check form-check-wrap">
            <md-checkbox
          ...
          <md-button
            type="submit"
            :class="{ 'btn-disabled': !btnActivated }"
            class="md-raised md-primary btn-full-btm"
            >Activate E-Commerce
            <md-progress-spinner
              v-if="requestLoading"
              class="md-accent btn-loading"
    ...
    <md-snackbar
        md-position="center"
        :md-duration="4000"
        :md-active.sync="showAppliedError"
        md-persistent
      >
        <span>{{ message }}</span>
        <md-button class="md-primary" @click="showAppliedError = false"
          >Close</md-button
        >
      </md-snackbar>
  </div>
...
export default {
  ...
  data() {
    return {
        message: "IPG already been requested.",
      showAppliedError: false,
      btnActivated: true,
      height: "",
      agreeTerms: false,
      pdfPath: pdfPath,
  ...
  methods: {
    changeSelectedCards() {
        if(this.canApplyCard) {
            this.form.cardType = "DEBIT";
        } else {
            this.form.cardType = "CREDIT";
        }
        
    },
    verifyAlreadyExist() {
      if (this.form.cardType == "DEBIT" && !this.canApplyCard) {
        this.showAppliedError = true;
        this.btnActivated = false;
      } else {
        this.btnActivated = true;
        this.showAppliedError = false;
      }
    },
